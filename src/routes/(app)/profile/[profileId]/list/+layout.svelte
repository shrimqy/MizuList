...
<nav class="listNav">
  <a
    ...
    class:selected={$page.url.pathname === "/profile/" + username + "/list/All"}
    >All</a
  >
  <a
    href="/profile/{username}/list/"
    class:selected={$page.url.pathname === "/profile/" + username + "/list"}
    >Reading</a
  >
  <a
    href="/profile/{username}/list/Planning"
    class:selected={$page.url.pathname ===
      "/profile/" + username + "/list/Planning"}>Plan to Read</a
  >
  <a
    href="/profile/{username}/list/Paused"
    class:selected={$page.url.pathname ===
      "/profile/" + username + "/list/Paused"}>Paused</a
  >
  <a
    href="/profile/{username}/list/Dropped"
    class:selected={$page.url.pathname ===
      "/profile/" + username + "/list/Dropped"}>Dropped</a
  >
  <a
    href="/profile/{username}/list/Completed"
    class:selected={$page.url.pathname ===
      "/profile/" + username + "/list/Completed"}>Completed</a
  >
</nav>
<a href="/edit/new">
  ...
  <div class="editor-popout" in:fade={{ duration: 300 }}>
    <form method="POST" action="/profile/{username}/list/addBook?/addBook">
      <div class="editor">
        <div class="ebutton">
          <button
            formaction="/profile/{username}/list/addBook?/addBook"
...
<style>
  * {
  ...
  .addButton:hover .material-symbols-round {
    ...
    left: calc(
      50% - 2.75rem
    ); /* Adjust the horizontal position to counter the increased width */
  }
