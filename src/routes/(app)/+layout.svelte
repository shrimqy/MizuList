...
<div class="header">
  ...
  {/if}
  <div
    class="navBar {$page.url.pathname.startsWith(`/profile/`)
      ? 'profile-page'
      : ''}"
  >
    <div class="nav">
          ...
          >
            ...
            {#if showrdrop}
              <div class="rdropdown" in:fade={{ duration: 200 }}>
                <div class="rdropLink">
                  <span class="material-icons"> recommend </span><a
                    href="/recommendations">Recommendations</a
                  >
                </div>
                <div class="rdropLink">
                  <span class="material-icons"> star </span><a href="/reviews"
                    >Reviews</a
                  >
                </div>
              </div>
            ...
            <div class="dropdown" in:fade={{ duration: 200 }}>
              <div class="dropLink">
                <span class="material-icons"> person </span><a
                  href="/profile/{homeusername}">Profile</a
                >
              </div>
              <div class="dropLink">
              ...
              <form action="/logout" method="POST">
                <div class="dropLink">
                  <span class="material-icons"> logout </span><button
...
{#if profilePage}
  <div class="banner">
    <div class="profile-header">
      <div class="user">
        {#if userData?.avatar}
          <img
            src={`/uploads/userAvatars/${userData.id}.png`}
            alt="User Avatar"
            class="userAvatar-profileNav"
          />
        {/if}
  ...
  <nav class="profileNav">
    <a
      ...
      class:selected={$page.url.pathname === `/profile/` + userData.username}
      >Overview</a
    >
    <a
      href="/profile/{userData.username}/list"
      class:selected={$page.url.pathname.startsWith(
        `/profile/` + userData.username + `/list`
      )}>List</a
    >
    <a
      href="/profile/{userData.username}/reviews"
      class:selected={$page.url.pathname.startsWith(
        `/profile/` + userData.username + `/reviews`
      )}>Reviews</a
    >
    ...
    {#if $page.url.pathname.startsWith(`/profile/` + username)}
      ...
      <a
        ...
        class:selected={$page.url.pathname.startsWith(
          `/profile/` + userData.username + `/history`
        )}>History</a
      >
      <a
        href="/profile/{userData.username}/social"
        class:selected={$page.url.pathname.startsWith(
          `/profile/` + userData.username + `/social`
        )}>Social</a
      >
